#!/usr/bin/env bash
# validate-panels.sh — structural validation of comic-v2 SVG panels
...
# Expected (file width height palette must_have_maya)
EXPECTED=(
  ...
  "11-the-stamp.svg 600 400 blue yes"
)
...
echo "Validating comic-v2 panels in $(pwd)"
echo "======================================"
...
for row in "${EXPECTED[@]}"; do
  ...
  # 4. Palette accent present (the dominant color hex appears in the SVG)
  hex=$(hex_for "$palette")
  if [ -z "$hex" ]; then
  ...
  if [ "$need_maya" = "yes" ]; then
    ...
    if [ "$has_maya" -gt 0 ]; then
      echo "WARN  $file — Maya should be absent but data-maya='present' found"
      # not a hard fail — Maya is allowed in climaxes
    fi
  fi

  # 6. SVG balance sanity (rough)
